...
class Stream extends EventEmitter{
    ...
    loop(){
        console.log("*** 2:", this.video_url)
        const end = this.bytes_count + this.per_sec_byte * 300;
        // Skipping callbacks sometimes???? Could explain song skips investigate later
        requestCallback(this.url, {
            headers: {
                range: `bytes=${this.bytes_count}-${end >= this.content_length ? '' : end}`
            }
        }, true).then(stream => {
            console.log("*** 3: stream", stream.statusCode, stream.statusMessage, this.video_url)
            if(Number(stream.statusCode) >= 400){
                console.log("*** 4: IS THIS SONG BEING SKIPPED???", this.video_url)
            ...
            stream.on('data', chunk => {
                try {
                    ...
                    console.log("*** Error on Stream.On", err)
                }
            })
            ...
            stream.on('end', () => {
                try {
                    ...
                    console.log("*** Error on Stream.End", err)
                }
                
            })
        }).catch(err => {
            console.log("*** Error interpolating stream", err)
            if (this.retryCount === 10) throw new Error(err);
            ++this.retryCount;
            return this.retry();
