    ...
    if (globals[player.channel.id].get(`settings`).youtube === false) {
      ...
      const resource = voice.createAudioResource(_stream, {
        inputType: ffmpeg
          ...
          : playable_stream.type === "webm/opus"
          ? voice.StreamType.WebmOpus
...
function idleCallback(channelId, guildId, playerC) {
  if (globals[channelId] instanceof ValueSaver) {
    ...
    if (globals[channelId].get(`settings`)["autoleave"] === true)
      if (voice.getVoiceConnection(guildId)) voice.getVoiceConnection(guildId).disconnect();
...
class Player extends EventEmitter {
  ...
  constructor(channel, options) {
    if (channel === undefined || typeof channel === "undefined" || channel === "")
      throw new Error(`A valid channel is required to provide as an argument`);
  ...
  play(stream, options) {
    return new Promise(async (resolve, reject) => {
      ...
      if (stream === undefined || typeof stream === "undefined" || stream === "")
        return reject(c`A valid channel is required to provide as an argument`);
  ...
  setFilter(...filters) {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.playing === false) resolve();
        ...
        this.play(currentStream, currentOptions)
          .then(() => resolve())
          .catch(reject);
      }
    });
