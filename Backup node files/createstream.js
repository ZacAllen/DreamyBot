...
function stream(ytstream, info, options){
    console.log("*** ENTERING STREAM", info)
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if(typeof info === 'string'){
            ...
            try{
                ...
                console.log("*** STREAM STEP 2")
            } catch (err) {
                console.log("*** STREAM STEP 2 ERROR", err, info)
                while (tries < 10) {
                    console.log("*** TRYING FETCH AGAIN", tries)
                    try {
                        _info = await getInfo(ytstream, info);
                        const _ci = await cipher.format_decipher(_info.formats, _info.html5player);
                        _info.formats = _ci;
            ...
            try{
                ...
                _info.formats = _ci;
                stream_res = await getStreamURL(_info, _options);
                console.log("*** STREAM STEP 3")
            } catch (err) {
                console.log("*** STREAM STEP 3 ERROR", err)
                return reject(err);
            }
